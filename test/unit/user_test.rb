...
class UserTest < Test::Unit::TestCase
  ...
  def test_should_not_rehash_password
    users(:quentin).update_attributes(:email => 'quentin2')
  ...
  def test_suspended_user_should_not_authenticate
    users(:quentin).suspend!
    assert_not_equal users(:quentin), User.authenticate('quire@example.com', 'test')
  end
...
protected
  def create_user(options = {})
    record = User.new({ :login => 'quire', :email => 'quire@example.com', :password => 'quire', :password_confirmation => 'quire', :real_name => 'quentin', :institution => 'uva', :role => 1 }.merge(options))
